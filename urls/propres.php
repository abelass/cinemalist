...
//revoi dees anciens urls	

	if (preg_match("/film/i",$i,$matches) OR preg_match("/acteur/i",$i,$matches) OR preg_match("/realisateur/i",$i,$matches) OR preg_match("/scenariste/i",$i,$matches)) {
		$explode=explode(',',$i);
		if($explode[1]){
			$objet=$matches[0];
			$id_objet = str_replace('/'.$objet,'',$explode[0]);
			$url_renvoi= sql_fetsel('url','spip_urls','id_objet='.$id_objet.' AND type="'.$objet.'"');
			if($url_renvoi['url'])$url_renvoi=$url_renvoi['url'];
			else $url_renvoi='spip.php?page='.$objet.'&id_'.$objet.'='.$id_objet;
			header('location:'.$url_renvoi.'.html');
			}	
		} 
